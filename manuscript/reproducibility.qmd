...
description: "Prospective Validation of Multi-week Inpatient Census Forecasting for Pediatric HSCT and Cellular Therapy"
author:
  - name: "Ezra Porter"
    affiliation: "Children's Hospital of Philadelphia"
  - name: "Richard S Hanna"
    affiliation: "Children's Hospital of Philadelphia"
...
This document provides computer code that was used to create the tables, figures, and results which we report in our manuscript, Prospective Validation of Multi-week Inpatient Census Forecasting for Pediatric HSCT and Cellular Therapy. We provide this document to allow other interested parties to inspect and verify our analytic work. Since the underlying data files contain extensive PHI, they are not publicly available and users will not be able to run this document themselves.

The first section lists concise descriptions of the steps performed in our [Analyses](#analyses). To view the code (written in the [R programming language](https://www.r-project.org/)) that was used to perform a specific step, as well as the results, click the black triangle to the left of the description to expand each section. The code relies heavily on the [`BMTCensusManuscript`](https://github.com/CHOP-CGTInformatics/BMTCensusManuscript) package developed to house code to reproduce the manuscript results. While the order of this section mirrors the order of results reported in the main manuscript, the section headings follow an analytical order laid out in the Materials and Methods section of the manuscript to allow greater depth in this document. Tables and figures from the main manuscript and supplementary materials are cross-referenced.

The second section describes the precise [Computational Environment](#computational-environment) in which the computational analyses were performed. Among other parameters, this part lists the names, versions, and sources of all software packages that were used to generate this document.
...
<summary>Schematic overview of census prediction model **(Figure 1)**</summary>
...
<summary>Plot representative example of census components **(Figure 2)**</summary>
...
fig_2$date <- as_date("2023-04-01")
fig_2$h <- 65

fig_2$base_data <- manuscript_census_data |>
  filter(time > 0, date == fig_2$date)

fig_2$current_admit_data <- fig_2$base_data |>
  extend_discharge_data(fig_2$h, end_var = bmt_team_end_date) |>
  mutate(date = fig_2$date + .h) |>
  summarize(value = sum(discharge == "No"), .by = date)

# Add in time 0 row
fig_2$current_admit_data <- bind_rows(
  count(fig_2$base_data, date, name = "value"),
  fig_2$current_admit_data
)

fig_2$new_admit_data <- admission_data |>
  filter(date == fig_2$date) |>
...
# Add in time 0 row
fig_2$new_admit_data <- bind_rows(
  tibble(date = fig_2$date, value = 0),
  fig_2$new_admit_data
)

fig_2$data <- inner_join(
  rename(fig_2$current_admit_data, current = value),
  rename(fig_2$new_admit_data, new = value),
  by = "date",
...
fig_2$data |>
  ...
  coord_cartesian(xlim = c(as_date(NA), fig_2$h - 5)) +
...
<details>
<summary>Plot length of stay by admission type **(Figure 3a)**</summary>
...
<summary>Plot daily census by admission type **(Figure 3b)**</summary>

```{r}
#| label: fig 3b

fig_3b_data <- manuscript_census_data |>
...
<summary>Generate admission counts by year and admission type **(Table 1)**</summary>
...
table_1$by_type <- manuscript_encounter_data |>
  mutate(year = year(bmt_team_start_date)) |>
  filter(year < 2023) |>
  count(year, admission_type_malig) |>
  pivot_wider(names_from = year, values_from = n)

table_1$total <- table_1$by_type |>
  summarize(
    admission_type_malig = "Total",
    across(-admission_type_malig, sum)
  )

bind_rows(table_1) |>
  rename(`Admission Type` = admission_type_malig) |>
  kbl() |>
  add_header_above(c(" ", "Number of Admissions (Calendar Year)" = ncol(table_1$total) - 1), bold = TRUE)
```
...
<summary>Generate descriptive statistics of LOS and admission rates **(Table 2)**</summary>
...
table_2$los_data <- manuscript_encounter_data |>
  summarize(
    median = median(los),
...
table_2$census_share_data <- manuscript_census_data |>
...
table_2$census_share_data |>
  arrange(admission_type_malig) |>
  left_join(table_2$los_data, by = "admission_type_malig") |>
  mutate(
...
<summary>Calculate evaluation metrics for test set and prospective validation **(Table 3)**</summary>
...
table_3$test <- make_metrics_table(manuscript_test_results) |>
  filter(Component == "Total Census") |>
  select(-Component)

table_3$validation <- make_metrics_table(validation_results) |>
  filter(Component == "Total Census") |>
  select(-Component)

table_3$thresholds <- tibble(
  Horizon = c(15, 30, 60),
  `Prospective Validation MAPE Threshold` = c(15, 20, 25),
  `Prospective Validation Result` = "Pass"
)

validation_estimates <- table_3$validation |>
  rename(.h = Horizon) |>
  pivot_longer(c(RMSE, MAPE), names_to = ".metric") |>
  mutate(.metric = tolower(.metric))

table_3$boot <- summarize_bootstrap(validation_results_bootstrap, validation_estimates)$table_empirical |>
  filter(.metric == "mape") |>
  select(Horizon = .h, `Prospective Validation MAPE 95% Bound` = `0.95`)

left_join(
  rename_with(table_3$test, \(x) paste("Test Set", x), c(RMSE, MAPE)),
  rename_with(table_3$validation, \(x) paste("Prospective Validation", x), c(RMSE, MAPE)),
  by = "Horizon"
) |>
  left_join(table_3$boot, by = "Horizon") |>
  left_join(table_3$thresholds, by = "Horizon") |>
  mutate(
...
<summary>Plot total census predictions for the test set **(Figure 4a)**</summary>
...
<summary>Plot total census predictions for the prospective validation **(Figure 4b)**</summary>
